...
summary.result = function(result, na.rm=T){
 ...
 # biashat = errcv-err
  ...
  ci3=cbind(errncv-biashat-qnorm(1-alpha/2)*sqrt(9/10)*sd.ncv, errncv-biashat+qnorm(1-alpha/2)*sqrt(9/10)*sd.ncv)
...
load("~/Dropbox/min/ncv/min/debugging/output/experiments/colon300.RData")
summary.result(exp_result, na.rm=T)


load("~/Dropbox/min/ncv/min/debugging/output/experiments/colon.RData")
summary.result(exp_result, na.rm=T)


```
reduce sample size as two rows per sample
```{r}
